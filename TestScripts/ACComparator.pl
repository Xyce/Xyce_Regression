...
my ($gsformat, $skipfooter, $debug, $help);
...
my $goldstandard=$ARGV[0];
my $testfile=$ARGV[1];
my $abstol=$ARGV[2];
my $reltol=$ARGV[3];   
my $zerotol=$ARGV[4];
my $freqreltol=$ARGV[5];
...
my ($GSFILEH, $TESTFILEH);
open($GSFILEH,$goldstandard);
open($TESTFILEH,$testfile);

my (@gsheaderdata,@testheaderdata);
my @gsdata=ReadDataFile($GSFILEH,\@gsheaderdata);
my @testdata=ReadDataFile($TESTFILEH,\@testheaderdata);


############################################
# Set variables based on --gsformat option 
############################################
my ($numdatarows, $freqCol);
if ($gsformat eq "xyceprn")
{
...
elsif ($gsformat eq "otherprn")
{
    ...
    elsif ($#testdata > $#gsdata)
    {
        ...
        # than a string.
        for (my $i=$#gsdata+1;$i<=$#testdata;$i++)
        {
...
for (my $i=0;$i<=$numdatarows;$i++)
{
   ...
   if ($gsdata[$i][$freqCol] != $testdata[$i][$freqCol])
   {
        ...
        {
            my $relDiff=abs($testdata[$i][$freqCol]-$gsdata[$i][$freqCol])/$gsdata[$i][$freqCol];
    ...
    for (my $j=2;$j<=$#{$gsdata[$i]};$j++)
    {
        ...
        if ($gsdata[$i][$j] != $testdata[$i][$j])
        {
	      ...
	      debugPrint "good $gsdata[$i][$j] and test data $testdata[$i][$j] both less than zeroTol\n";
            ...
            {
	        my $absDiff = abs($testdata[$i][$j] - $gsdata[$i][$j]);
                my $relDiff = $absDiff / abs($gsdata[$i][$j]);   
