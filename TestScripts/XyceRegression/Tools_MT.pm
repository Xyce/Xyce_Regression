...
sub wrapXyceAndVerifyPLGS {
  ...
  $retval = $self->wrapXyce($XYCEPATH,$ckt,$cktprn);
  if ($retval == 0) {
    my $CMD = "$GOLDPL $wdirPrefix$cktprn";
    if( defined($self->getWorkDir()) ) {
    ...
    if ($retval == 0) {
      my $CMD="$XYCEVERIFY $wdirPrefix$ckt $wdirPrefix$ckt.prn.gs $wdirPrefix$cktprn > $wdirPrefix$ckt.prn.out 2>$wdirPrefix$ckt.prn.err";
      if( defined($self->getWorkDir()) ) {
        $CMD= "cd $wdirPrefix; " . $CMD;
      }
      $retval = system("$CMD");
...
sub runXyce
{
  ...
  fork:
  {
    ...
    elsif (defined $pid)
    {
      ...
      #open STDOUT, ">$wdirPrefix$cktnoquotes.out" or die "Child $$:  Can't redirect STDOUT: $!";
      $self->debugPrint("Child $$:  Redirecting STDERR to $cktnoquotes.err\n");
      #open STDERR, ">$wdirPrefixcktnoquotes.err" or die "Child $$:  Can't redirect STDERR: $!";
      if( defined($self->getWorkDir()) ) {
        exec "cd $wdir;$XYCEPATH $ckt > $cktnoquotes.out 2> $cktnoquotes.err";
      } 
      else {
        exec "$XYCEPATH $ckt > $cktnoquotes.out 2> $cktnoquotes.err";
      }
...
sub wrapShellScript
{
  ...
  return $self->wrapExec($execstr,"$ckt.sh.stdout","$ckt.sh.stderr",$self->{childwait});
}
...
sub forkCommand
{
  ...
  fork:
  {
    ...
    elsif (defined $pid)
    {
      ...
      if( defined($self->getWorkDir()) ) {
        my $wdir = $self->getWorkDir();
        exec "cd $wdir; $execstr > $stdout_file 2> $stderr_file";
      }
      else {
        exec "$execstr > $stdout_file 2> $stderr_file";
      }
      exit 0; # this is for when exec fails
    }
    elsif ($! == EAGAIN)
...
sub checkGroupedError
{
  ...
  $self->debugPrint("XyceRegression::Tools::checkGroupedError:  \$errfile = $wdirPrefix$errfile\n");
