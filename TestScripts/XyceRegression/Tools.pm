...
sub wrapXyce
{
  ...
  if ( -f "$ckterr" )
  {
    open(errfile,"$ckterr") or warn "WARNING:  Cannot open $ckterr\n";
    while ($line = <errfile>)
    {
  ...
  if ( ($exitcode == -10) and (-f "$cktout") )
  {
    open(outfile,"$cktout") or warn "WARNING:  Cannot open $cktout\n";
  ...
  if ($exitcode == -10) # no "Xyce Abort" message in $ckterr or $cktout
  {
    if ( (-f "$cktprn") )
    {
      ...
      my $prnstatus = `grep "End of Xyce[(]TM[)]" "$cktprn"`;
      my $prnstatus2 = `grep "#;" "$cktprn"`;
    ...
    {
      $self->debugPrint("No $cktprn file found, exiting with 14\n");
...
sub wrapShellScript
{
  ...
  my $execstr = "$SHELLFILE \'$XYCEPATH\' $XYCE_VERIFY $XYCE_COMPARE $ckt $GOLDPRNFILE $globaltaglist";
  system("chmod +x $SHELLFILE");
  return $self->wrapExec($execstr,"$ckt.stdout","$ckt.stderr",$self->{childwait});
}
...
sub forkCommand
{
  ...
  my $pid;
  use Errno qw(EAGAIN);
  $self->debugPrint("XyceRegression::Tools::forkCommand \$execstr = >>$execstr<<\n");
  $self->debugPrint("XyceRegression::Tools::forkCommand \$stdout_file = >>$stdout_file<<\n");
  $self->debugPrint("XyceRegression::Tools::forkCommand \$stderr_file = >>$stderr_file<<\n");
  fork:
  {
    if ($pid=fork) 
    {
      $self->debugPrint("XyceRegression::Tools::forkCommand:  This is the parent, returning pid = $pid\n");
      return $pid
    }
    elsif (defined $pid)
    {
      $self->debugPrint("XyceRegression::Tools::forkCommand:  Child $$:  This is the child with pid $$.\n");
      #close(STDOUT);
      #close(STDERR);
      setpgrp(0,0); # This sets process group so I can kill this + children
      $self->debugPrint("XyceRegression::Tools::forkCommand:  Child $$:  \$execstr = >>$execstr<<\n");
      $self->debugPrint("XyceRegression::Tools::forkCommand:  Child $$:  Redirecting STDOUT to $stdout_file\n");
      $self->debugPrint("XyceRegression::Tools::forkCommand:  Child $$:  Redirecting STDERR to $stderr_file\n");
...
sub runAndCheckWarning
{
  ...
  if ($retval != 0) { return $retval; } # In this case, Xyce should NOT EXIT WITH ERROR
...
sub runAndCheckError
{
  ...
  if ($retval == 0) { return 2; } # In this case, Xyce should EXIT WITH ERROR, and not doing so is a failure
...
sub checkGroupedError
{
  ...
  for $searchIdx (0 .. $#searchlist)
  {
    ...
    for $lineIdx (0 .. $#lines)
    {
	  ...
	  $matchCount++;
        ...
        {
          # If no match, then reset the $matchCount variable and keep searching.
          # through the @lines array.  This accounts for multiple error messages 
          # from the same netlist line being emitted in a different ordering in 
          # parallel, or with some "MPI abort" text appearing between messages 
          # from the same netlist line.  See SON Bug 951 for more details.
	  $matchCount=0;
          last;
        }   
      }
  ...
  {
    my $numLines = $#searchlist+1;
    print "Number of matches ($found) not equal to number of grouped lines ($numLines) in .sh file\n";
    print "Test Failed!\n";
    return 2;
  }
}
