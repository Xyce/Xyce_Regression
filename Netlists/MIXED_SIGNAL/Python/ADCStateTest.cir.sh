...
@searchstrings = ("return value from getADCWidths is 1",
                  "\\[3, 2\\]",
                  "simulateUntil status = 1 and actual_time = 0.00001",
                  "return value from getTimeStatePairsADC is 1",
                  "number of ADC names returned by getTimeStatePairsADC is 2",
...
# now check the correctness of the timeArray and stateArray values
$fc = $XYCE_VERIFY;
...
print ("Testing timeArray and stateArray values\n");
$CMD="$fc $CIRFILE.TSarrayData $CIRFILE.TSarrayGoldData $absTol $relTol $zeroTol";
if (system($CMD) != 0) 
{
  print STDERR "Verification failed on file $CIRFILE.TSarrayData";
  $retval = 2;
}
