...
if (not -s "$CIR0.all.SENS.prn" )
{
  print "$CIR0.all.SENS.prn file is missing\n";
  print "Exit code = 14\n";
  exit 14;
}
...
if (not -s "$CIR0.separator.SENS.prn" )
{
  print "$CIR0.separator.SENS.prn file is missing\n";
  print "Exit code = 14\n";
  exit 14;
}
...
{
  print "$CIR1.SENS.prn file is missing\n";
  print "Exit code = 14\n";
  exit 14;
}
...
system("tail -n 2 $CIR0.all.SENS.prn > $CIR0.all.noheader.SENS.prn");

system("tail -n 2 $CIR1.prn > $CIR1.noheader.prn");
system("tail -n 2 $CIR1.SENS.prn > $CIR1.noheader.SENS.prn");
...
$CMD_DIFF="diff $CIR0.all.noheader.SENS.prn $CIR1.noheader.SENS.prn > $CIRFILE.all.out 2> $CIRFILE.all.err";
if (system("$CMD_DIFF") != 0) { $retcode=2; print STDERR "$CMD_DIFF failed\n";}

$CMD_DIFF="diff $CIR0.separator.prn $CIR0.all.prn > $CIRFILE.separator.out 2> $CIRFILE.separator.err";
if (system("$CMD_DIFF") != 0) { $retcode=2; print STDERR "$CMD_DIFF failed\n";}

$CMD_DIFF="diff $CIR0.separator.SENS.prn $CIR0.all.SENS.prn > $CIRFILE.separator.out 2> $CIRFILE.separator.err";
if (system("$CMD_DIFF") != 0) { $retcode=2; print STDERR "$CMD_DIFF failed\n";}
