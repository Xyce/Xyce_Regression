...
system("rm -f $CIRFILE.NOISE.prn $CIRFILE\_*.ma0");
...
# to stdout to the "gold" stdout in the $GSFILE (NoiseGSfile).
...
# with the "Gold" .ma0 file, which is in OutputData/MEASURE_CONT/USE_CONT_FILES
...
{
  $CONTMA0 = "$CIRFILE\_$contMeasureNames[$i].ma0";
  $GOLDCONTMA0 = $GOLDPRN;
  $GOLDCONTMA0 =~ s/.prn$//;
  $GOLDCONTMA0 = "$GOLDCONTMA0\_$contMeasureNames[$i].ma0";

  if (not -s "$CONTMA0" )
  {
    print "$CONTMA0 file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  if (not -s "$GOLDCONTMA0" )
  {
    print "Gold $CONTMA0 file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  $CMD="$fc $CONTMA0 $GOLDCONTMA0 $absTol $relTol $zeroTol >> $CIRFILE.errmsg.out 2>> $CIRFILE.errmsg.err";
  $retval=system($CMD);
  $retval= $retval >> 8;

  if ( $retval != 0 )
  {
    print STDERR "test failed comparison of Gold and measured .ma0 files for $CONTMA0 with exit code $retval\n";
    $exitcode=2;
  }
  else
  {
    print "Passed comparison of $CONTMA0 file\n";
  }
}
