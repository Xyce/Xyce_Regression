...
system("rm -f $CIRFILE.mt0 $CIRFILE.prn $CIRFILE.out $CIRFILE.err* $CIRFILE.remeasure*");
...
# to stdout to the "gold" stdout in the $GSFILE (TranGSfile).
...
# with the "Gold" .mt0 file, which is in OutputData/MEASURE_CONT/USE_CONT_FILES
...
$retval = MeasureCommon::checkRemeasure($XYCE,$XYCE_VERIFY,$CIRFILE,$absTol,$relTol,$zeroTol,'prn',1,'mt',$contMeasureNamesRef);
