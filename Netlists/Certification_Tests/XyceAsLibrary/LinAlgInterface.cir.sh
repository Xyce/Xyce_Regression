...
$MAKEROOT = "$XYCEROOT/src/test/LinearAlgebraTest/";
#UGH
...
if (-d "$MAKEROOT") {
  if (-e "$MAKEROOT/Makefile") {
    ...
    print "NOTICE:   make testBlockLinearSystems ------------------\n";
    $result += system("make testBlockLinearSystems$EXT");
    if($result) {
...
if (-x $XYCE_LIBTEST) 
{
  ...
  if($retval != 0) 
  { 
    print "ERROR:    LinearAlgebraTest failures! --------\n"; 
    $returncode = $retval >> 8;
