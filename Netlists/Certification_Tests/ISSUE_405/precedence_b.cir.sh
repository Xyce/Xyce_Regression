...
$LET=$CIRFILE;
$LET =~ s/precedence_([a-z])\.cir/$1/;

$cirfiles = `ls precedence_${LET}[0-9].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  ...
  $CMD = "$XYCE -redefined_params uselast $cirname > $cirname.out 2> $cirname.err";
  if ( system("$CMD") != 0 ) 
  { 
...
foreach $cirname (@cirlist)
{
  $CMD = "diff precedence_${LET}0.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY precedence_${LET}0.cir precedence_${LET}0.cir.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
...
{ 
  `echo "Test Failed!" >> precedence_$LET.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "precedence_$LET.cir.prn.err" ) { `rm -f precedence_$LET.cir.prn.err`; }
`echo "Test Passed!" >> precedence_$LET.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
