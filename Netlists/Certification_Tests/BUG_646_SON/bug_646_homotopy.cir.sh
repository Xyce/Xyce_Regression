...
foreach $idx (3 .. 5)
{ 
  `rm -f $CIRFILE$idx.csd`; 
}

# create the output files
$CMD="$XYCE $CIRFILE > $CIRFILE.out 2>$CIRFILE.err";
$retval=$retval=system($CMD);
if ($retval != 0) 
{
  if ($retval & 127)
  {
...
# check each output file against it's corresponding "gold .CSD file".  Those
# gold files are currently only made for 3, 4 and 5 variables on the 
# .PRINT AC FORMAT=PROBE line.
foreach $idx (3 .. 5)
{
  # file names
  $testFileName="$CIRFILE$idx.csd";
  $goldFileName="$CIRFILE$idx.GSfile";
  ...
  {
    print "test passed for $idx variables on .PRINT line\n";
  }
}
