...
$cirfiles = `ls bug_1692_[0-9].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  $CMD = "diff bug_1692_${LET}0.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY bug_1692_0.cir bug_1692_0.cir.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
...
{ 
  `echo "Test Failed!" >> bug_1692.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "bug_1692_$LET.cir.prn.err" ) { `rm -f bug_1692_$LET.cir.prn.err`; }
`echo "Test Passed!" >> bug_1692_$LET.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
