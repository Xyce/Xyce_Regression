...
@FD_OUT;
@TD_OUT;
$CIR[0]="ac_files_csv.cir";
$FD_OUT[0]="ac_bleem_csv";
$TD_OUT[0]="ac_bleem_td_csv";

$CIR[1]="ac_files_fallback_csv.cir";
$FD_OUT[1]="ac_foo_csv";
$TD_OUT[1]="ac_foo_csv.TD.csv";
...
foreach $idx (0 .. 1)
{
  $CMD="$fc $TD_OUT[$idx] $GOLDCSV.TD.csv $abstol $reltol $zerotol > $TD_OUT[$idx].out 2> $TD_OUT[$idx].err";
  if (system("$CMD") != 0) {
