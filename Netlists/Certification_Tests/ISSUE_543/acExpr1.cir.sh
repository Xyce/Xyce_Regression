...
system("rm -f $CIRFILE2.FD.prn $CIRFILE2.out $CIRFILE2.err");

# run Xyce on test file
...
#if (-f "$CIRFILE.FD.prn" && -f "$CIRFILE2.FD.prn")
if (-f "$CIRFILE.FD.prn" )
{
  $GOLDPRN = "$CIRFILE2.FD.prn";

  $CMD="$XYCE_VERIFY --gsformat=otherprn $GOLDPRN $CIRFILE.FD.prn $abstol $reltol $zerotol $freqreltol";
  $retval = system($CMD);
  $retval = $retval >> 8;
  if ($retval == 0) 
  { 
