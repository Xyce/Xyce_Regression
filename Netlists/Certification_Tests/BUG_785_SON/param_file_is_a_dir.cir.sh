...
# execute Xyce to read a parameters file (via -prf), where the requested file is 
# actually a directory
$CMD="$XYCE -prf . -rsf response1.out $CIRFILE > $CIRFILE.out 2>$CIRFILE.err";
$retval=system($CMD);
if ($retval == 0)
{
...
@searchstrings = ("Could not find parameter file: \\.  Attempting to continue\\.");
