...
# execute Xyce to make a names file, where the requested file is in
# a subdirectory that does not exist.  Xyce should run to completion,
# with a warning message.
$CMD="$XYCE -namesfile bogo_dir/namesfile.txt $CIRFILE > $CIRFILE.out 2>$CIRFILE.err";
$retval=system($CMD);
if ($retval != 0) 
{
  if ($retval & 127)
