...
# execute Xyce to try to make a response file (via -rsf) in a 
# non-existent subdirectory.
$CMD="$XYCE -prf params.in -rsf bogo_dir/response.out $CIRFILE > $CIRFILE.out 2>$CIRFILE.err";
$retval=system($CMD);
if ($retval == 0)
{
...
@searchstrings = ("Netlist error: Could not open response file bogo_dir/response\\.out for output");
