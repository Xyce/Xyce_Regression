...
$CIRFILE="testgauss.cir";

$mean_requested = 100;
$abs_dev = 10;   # gauss returns random deviates around mean +/- abs_dev, not
                 # +/- infinity.
$rel_dev= $abs_dev/$mean_requested;  # gauss returns +/- rel_dev*mu
$sigma_unit = 10; # gauss uses abs_dev/sigma_unit as the standard
...
print CIROUT "*TEST of gaussian random deviates with GAUSS instead of AGAUSS\n";
print CIROUT "I1 1 0 DC -1\n";

for ($i=1;$i<=$NUM_RANDS;$i++)
{
    print CIROUT "R$i 1 0 {gauss($mean_requested,$rel_dev,$sigma_unit)}\n";
}
