...
$CIRFILE_modelpar="baselineLumpedLine.cir";
...
{
    # Change header of baselineLumpedLine to match lumpedline
    `sed -e 's/RCABLE/TESTLINE:R/' -e 's/CCABLE/TESTLINE:C/' -e 's/LCABLE/TESTLINE:L/' < $CIRFILE_modelpar.prn > $CIRFILE_modelpar.modded.prn`;
  # use file_compare since xyce verify doesn't like NOINDEX format.
    ...
    `$fc $CIRFILE.prn $CIRFILE_modelpar.modded.prn $absTol $relTol $zeroTol > $CIRFILE.prn.out 2> $CIRFILE.prn.err`;  
    $retval=$? >> 8;
}
