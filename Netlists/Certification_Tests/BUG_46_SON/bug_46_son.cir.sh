...
$cirfiles = "baseline.cir synonyms.cir";
...
foreach $cirname (@cirlist)
{
  $CMD = "diff baseline.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
...
{ 
  `echo "Test Failed!" >> bug_46_son.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "bug_46_son.cir.prn.err" ) { `rm -f bug_46_son.cir.prn.err`; }
