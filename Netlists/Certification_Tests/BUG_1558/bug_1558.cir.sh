...
$cirfiles = `ls bug_1558_[0-9].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  $CMD = "diff bug_1558_${LET}0.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
...
{ 
  `echo "Test Failed!" >> bug_1558.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "bug_1558_$LET.cir.prn.err" ) { `rm -f bug_1558_$LET.cir.prn.err`; }
`echo "Test Passed!" >> bug_1558_$LET.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
