...
foreach $cirname (@cirlist)
{
  ...
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY test_${LET}0.cir.prn $cirname.prn $abstol $reltol $zerotol >> $cirname.prn.out 2>> $cirname.prn.err";
    `echo "diff failed, now trying file_compare.pl " >> $cirname.prn.err`;
...
`echo "Test Passed!" >> test_$LET.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
