...
else
{
  # use file_compare since xyce verify doesn't like NOINDEX format.
    ...
    `$fc $CIRFILE.prn $CIRFILERef.prn $absTol $relTol $zeroTol > $CIRFILE.prn.out 2> $CIRFILE.prn.err`;  
    $retval=$? >> 8;
}
