...
$BASECIR="tran-nohomotopy.cir"; 

# these files have various .PRINT SENS lines in them.
@CIR;
$CIR[0]="tran-homotopy.cir";
$CIR[1]="tran-homotopy-multifile.cir";
$CIR[2]="tran-homotopy-nooverwrite.cir";

$DASHR[0]="$CIR[0].raw";
$DASHR[1]="$CIR[1].raw";
$DASHR[2]=$CIR[2];

$CSV[0]="$CIR[0].HOMOTOPY.csv"; 
$CSV[1]="tran-sens-multifile.HOMOTOPY.csv";
$CSV[2]="$CIR[2].HOMOTOPY.csv";
...
if ( -f "$BASECIR.HOMOTOPY.csv" ) {
  print STDERR "Extra output file $BASECIR.HOMOTOPY.csv\n";
  $xyceexit=2;
}
...
# now run the netlists with .PRINT HOMOTOPY lines in them
foreach $idx (0 .. 2)
{
...
foreach $idx (0 .. 2)
{
  print "testing files for $CIR[$idx]\n";
  ...
  $CMD="$fc $CSV[$idx] $GOLDCSV.HOMOTOPY.csv $abstol $reltol $zerotol > $CSV[$idx].out 2> $CSV[$idx].err";
  if (system($CMD) != 0) {
  ...
  {
    print "passed comparison of .csv files\n";
  }
}
