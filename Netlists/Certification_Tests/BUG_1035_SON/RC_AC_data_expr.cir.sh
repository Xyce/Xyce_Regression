...
$CIR1="RC_AC_data_expr.cir";
$CIR2="RC_AC_data_expr2.cir";
$CIR3="RC_AC_data_expr3.cir";
...
system("rm -f $CIR3.FD.prn $CIR3.out $CIR3.err");

# run Xyce on all netlists
...
# Do the comparisons.  For all of them, assume CIR1 is the "baseline" result.
...
# compare CIR3
# do a "diff" comparison first
$CMD="diff $CIR1.FD.prn $CIR3.FD.prn > $CIR1.FD.prn.out 2> $CIR1.FD.prn.err";
$retval = system($CMD);
if ($retval != 0)
{
  # if "diff" fails, then do a Xyce verify comparison
  $CMD="$XYCE_VERIFY  --gsformat=xyceprn $CIR1.FD.prn $CIR3.FD.prn $abstol $reltol $zerotol $freqreltol";
  $retval = system($CMD);
  $retval = $retval >> 8;
  if ($retval == 0) 
  { 
