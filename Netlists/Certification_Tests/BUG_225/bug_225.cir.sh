...
$line = <OUT>;
if (not ($line =~ m/^TITLE=\'\* bug_225.cir\'/i)) { $failure=1; }
$line = <OUT>;
if (not ($line =~ m/^SUBTITLE=\'Xyce data\'/)) { $failure=1; }
$line = <OUT>;
if (not ($line =~ m/^TIME=.*\s+DATE=.*\s+TEMPERATURE=/)) { $failure=1; }
$line = <OUT>;
if (not ($line =~ m/^ANALYSIS=\'DC Sweep\'\s+SERIALNO=\'12345\'/)) { $failure=1; }
$line = <OUT>;
if (not ($line =~ m/^ALLVALUES=\'NO\'\s+COMPLEXVALUES=\'NO\'\s+NODES=\'2\'/)) { $failure=1; }
$line = <OUT>;
if (not ($line =~ m/^SWEEPVAR=\'V1\'\s+SWEEPMODE=\'VAR_STEP\'/)) { $failure=1; }
$line = <OUT>;
...
$line = <OUT>;
if (not ($line =~ m/^\'V\(1\)\'\s+\'I\(V1\)\'/)) { $failure=1; }
