...
`rm -rf $CIRFILE_baseline*.prn $CIRFILE*.err $CIRFILE*.out`;
...
else
{
    ...
    `$fc $CIRFILE.prn $CIRFILE_baseline.prn $absTol $relTol $zeroTol > $CIRFILE.prn.out 2> $CIRFILE.prn.err`;
    $retval=$? >> 8;
    `$fc $CIRFILE.HOMOTOPY.prn $CIRFILE_baseline.HOMOTOPY.prn $absTol $relTol $zeroTol >> $CIRFILE.prn.out 2>> $CIRFILE.prn.err`;
    $retval2=$? >> 8;
