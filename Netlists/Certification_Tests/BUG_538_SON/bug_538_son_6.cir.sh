...
else
{
    ...
    `$fc $CIRFILE.FD.prn $CIRFILE_baseline.FD.prn $absTol $relTol $zeroTol > $CIRFILE.prn.out 2> $CIRFILE.prn.err`;
    $retval=$? >> 8;
    `$fc b538_6 b538_6_bl $absTol $relTol $zeroTol > $CIRFILE.prn.out 2> $CIRFILE.prn.err`;
    $retval2=$? >> 8;
