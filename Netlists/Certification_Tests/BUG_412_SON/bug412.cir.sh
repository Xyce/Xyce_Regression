...
while( $line=<NETLIST> )
{
  ...
  {  
    ...
    # caution! must match case in netlist
    @words = split( /\s+/, $line);
    foreach $word (@words)
    {
      if( $word =~ "=" )
      {
        ...
        # caution! need to match case in the netlist
        if( $keyword eq "on" )
        {
        ...
        elsif( $keyword eq "cross" )
        {
          @crossVals[$numMeasures-1] = $value;
        }
        else
        {
...
# For this measure we need to find the time when a given volage 
# equaled or exceeded a set value.  Note if "rise" is specified such 
# as "rise=2" when we want the second time when the signal reached 
# that value. if 'cross=n' then we find the n't time we cross that value.
...
{  
  ...
  for $j (0 .. 1)
  {
    #if ( (($dataFromXyce[$i-1][1] < $findVals[$j] ) && ($dataFromXyce[$i][1] > $findVals[$j] )) && ($numberOfRises[$j] == $riseVals[$j]) )
    if ( (($dataFromXyce[$i-1][2] < $findVals[$j] ) && ($dataFromXyce[$i][2] > $findVals[$j] )) ||
         (($dataFromXyce[$i-1][2] > $findVals[$j] ) && ($dataFromXyce[$i][2] < $findVals[$j] )) )
    {
      $numberOfCrosses[$j] += 1;
      if( $numberOfCrosses[$j] == $crossVals[$j] )
      {
        # interpolate between the bounding points. 
        $calMeasure[$j] = $dataFromXyce[$i][1] - ( ($dataFromXyce[$i][1] - $dataFromXyce[$i-1][1])/($dataFromXyce[$i][2] - $dataFromXyce[$i-1][2])) * ($dataFromXyce[$i][2] - $findVals[$j]);
        # before interpolation we just tool the right sided limit
        # $calMeasure[$j] = $dataFromXyce[$i-1][1];
      }
    }
  }
}
...
for $i (0 .. 1)
{
  #$calMeasure[$i] /= $dataFromXyce[$#dataFromXyce-1][0];
