...
while( <DIAGF> )
{
  ...
  {
    $retval = 2;
    print( "Found a voltage output block when there shouldn't be one: $word[$#word]\n");
    print(  "Exit code = $retval\n" );
    exit 2;
  }
  if( $blockType eq 'I')
  {
    $retval = 2;
    print( "Found a current output block when there shouldn't be one: $word[$#word]\n");
    print(  "Exit code = $retval\n" );
    exit 2;
  }
  if( ($blockType eq 'S') && ($word[2] eq "DC_OP_SOURCE_STEPPING"))
  {
    $FoundSuccessInfo = 1;
  }
  $i = $i + 1;
...
if( $FoundSuccessInfo == 0 )
{
  print( "Did not find required diagnostic output\n");
  $retval = 2;
}
