...
`rm -f $CIRFILE.SENS.prn $CIRFILE.csd`;
...
# now make sure that the netlist file did not get overwritten.  This was a specific
# error that happened during early development of this feature.
$CMD_DIFF="diff  $CIRFILE $CIRFILE.backup > $CIRFILE.backup.out 2> $CIRFILE.backup.err";
if (system("$CMD_DIFF") != 0) 
{ 
  print "Circuit file corrupted or overwritten by .SENS output!\n";
  print "Exit code = 2\n"; exit 2;
}
