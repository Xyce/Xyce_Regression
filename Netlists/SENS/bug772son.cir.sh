...
if ( not -s "$CIR1.prn" ) 
{ 
  print STDERR "$CIR1.prn not found\n";
  print "Exit code = 14\n"; 
  exit 14; 
}
...
if ( not -s "$CIR2.prn" ) 
{ 
  print STDERR "$CIR2.prn not found\n";
  print "Exit code = 14\n"; 
  exit 14; 
}
...
`rm -f $CIR1.prn.out $CIR1.prn.err`; 
$retval = system("diff $CIR1.prn $CIR2.prn > $CIR1.prn.out 2> $CIR1.prn.err");
if ($retval == 0) 
{
  print "Exit code = 0\n";
  exit 0;
...
{
   $CMD = "$XYCE_VERIFY $CIR1.prn $CIR2.prn $abstol $reltol $zerotol >> $CIR1.prn.out 2>> $CIR1.prn.err";
   print STDERR "diff failed on $CIR1.prn $CIR2.prn\nRunning file_compare instead\n";
   if ( system("$CMD") != 0 )
   {
     print STDERR "Verify failed\n";
     print "Exit code = 2\n"; 
