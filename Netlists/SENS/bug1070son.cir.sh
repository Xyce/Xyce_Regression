...
$CIR1 = "sensCapGear_global2.cir";
$CIR2 = "sensCapGear_global_curlybrace.cir";
...
$retval = system("diff $CIR1.SENS.prn2 $CIR2.SENS.prn2 > $CIR1.SENS.prn.out 2> $CIR1.SENS.prn.err");
if ($retval == 0) 
{
  print "Exit code = 0\n";
  exit 0;
...
else
{
   ...
   print STDERR "diff failed on $CIR1.SENS.prn2 $CIR2.SENS.prn2\nRunning file_compare instead\n";
   if ( system("$CMD") != 0 )
   {
