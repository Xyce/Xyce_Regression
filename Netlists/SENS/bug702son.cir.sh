...
$retval = system("diff $CIR1.SENS.prn2 $CIR1.prn2 > $CIR1.SENS.prn.out 2> $CIR1.SENS.prn.err");
if ($retval == 0) 
{
  print "Exit code = 0\n";
  exit 0;
...
{
   $CMD = "$XYCE_VERIFY $CIR1.SENS.prn2 $CIR1.prn2 $abstol $reltol $zerotol >> $CIR1.SENS.prn.out 2>> $CIR1.SENS.prn.err";
   print STDERR "diff failed on $CIR1.SENS.prn2 $CIR1.prn2\nRunning file_compare instead\n";
   if ( system("$CMD") != 0 )
   {
