...
$cirfiles = "$BASE_FILENAME.cir $BASE_FILENAME"."_noFlip_P.cir";
...
# Now try diff of the noFlip_P circuit against the baseline:
...
foreach $cirname (@cirlist)
{
    ...
    if (system("$CMD") != 0)
    {
        ...
        $CMD="$XYCE_VERIFY --goodres=$CIRFILE.res.gs --testres=$cirname.res $cirname $CIRFILE.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
