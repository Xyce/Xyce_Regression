...
$cirfiles = `ls invert_psp_testDTA[0-9].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  $CMD = "diff invert_psp_testDTA0.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY invert_psp_testDTA0.cir invert_psp_testDTA0.cir.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
