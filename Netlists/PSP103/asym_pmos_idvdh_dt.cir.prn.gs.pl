...
$input_basename="asym_pmos_idvdh_dt_T";
$input_suffix=".standard";

$outname="asym_pmos_idvdh_dt.cir.prn.gs";
...
for $temp (@temps)
{
    print STDERR "doing $temp\n";
    $inputfile=$input_basename.$temp.$input_suffix;
    open (INPFILE,"<$inputfile") || die "cannot open $inputfile for read";
    print STDERR "Opened $inputfile\n";
    $line=<INPFILE>;
    print STDERR "skipping first line $line\n";
    $indx=0;
    for $vg (@vgs)
    {
        print STDERR "Doing vg=$vg\n";
        for ($vd=$vdstart;$vd>$vdstop+$vdincrement;$vd+=$vdincrement)
        {
            print STDERR "Doing vd=$vd\n";
            $line = <INPFILE>;
            chomp $line;
            print STDERR "got input line $line\n";
            ($vd_in,$vdt_in)=split(" ",$line);
            print STDERR "got $vd_in, $vdt_in\n";
            if ($vd_in - $vd > 0.001)
            {
