...
FILE_COMPARE="$(echo $XYCE_VERIFY | sed 's!\(.*\)/.*!\1!')/file_compare.pl"
absTol=1e-5
relTol=1e-3
zeroTol=1e-20

CIR1=continuation1.cir
CIR2=continuation2.cir
CIR3=continuation3.cir
CIR4=continuation4.cir
...
$FILE_COMPARE $CIR3.prn2 $CIR4.prn2 $absTol $relTol $zeroTol > $CIR3.prn.out 2> $CIR3.prn.err
if [ "$?" -ne "0" ]; then
