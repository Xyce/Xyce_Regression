...
CIR1=continuation1_mos1.cir
CIR2=continuation2_mos1.cir
CIR3=continuation3_mos1.cir
CIR4=continuation4_mos1.cir

# first comparison
$XYCE $CIR1 > $CIR1.out 2> $CIR1.err
if [ "$?" -ne "0" ]; then
  echo "$XYCE $CIR1 failed"
  echo "Exit code = 10"
  exit 10
fi
if [ ! -f $CIR1.HOMOTOPY.prn ]; then
  echo "$CIR1.HOMOTOPTY.prn missing"
  echo "Exit code = 14"
  exit 14
fi
...
if [ "$?" -ne "0" ]; then
  echo "$XYCE $CIR2 failed"
  echo "Exit code = 10"
  exit 10
fi
if [ ! -f $CIR2.HOMOTOPY.prn ]; then
  echo "$CIR2.HOMOTOPTY.prn missing"
  echo "Exit code = 14"
  exit 14
fi
...
if [ "$?" -ne "0" ]; then
  echo "$XYCE $CIR3 failed"
  echo "Exit code = 10"
  exit 10
fi
if [ ! -f $CIR3.HOMOTOPY.prn ]; then
  echo "$CIR3.HOMOTOPTY.prn missing"
  echo "Exit code = 14"
  exit 14
fi
...
if [ "$?" -ne "0" ]; then
  echo "$XYCE $CIR4 failed"
  echo "Exit code = 10"
  exit 10
fi
if [ ! -f $CIR4.HOMOTOPY.prn ]; then
  echo "$CIR4.HOMOTOPTY.prn missing"
  echo "Exit code = 14"
  exit 14
fi
