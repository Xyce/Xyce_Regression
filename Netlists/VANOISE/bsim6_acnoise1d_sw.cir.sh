...
$GOLDPRN =~ s/\.prn$/.NOISE.prn/; # insert ".NOISE" between base and .prn

if (-f "$CIRFILE.NOISE.prn")
{
  $CMD="$XYCE_VERIFY $GOLDPRN $CIRFILE.NOISE.prn $abstol $reltol $zerotol $freqreltol"; 
  $retval = system($CMD);
  $retval = $retval >> 8;
  if ($retval == 0) 
  { 
