...
# to stdout to the "gold" stdout in the $GSFILE (RMSTestGSfile).  This
...
foreach $stepNum (1 .. $numSteps)
{
  ...
  # with the "Gold" .maX file, which is in OutputData/MEASURE_AC/STEP/RMSTest.cir.maX
  # Check that the Gold .maX file exists
  $maSuffix=$stepNum-1;
  $rmsString= "ma$maSuffix";
  $GOLDRMS = $GOLDPRN;
  $GOLDRMS =~ s/prn$/$rmsString/;
  #print "GOLDRMS file = $GOLDRMS\n";
  $maSuffix = $stepNum-1;
  if (not -s "$GOLDRMS" )
  {
    print "GOLD $rmsString file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  # compare gold and measured .ma files, for this step.
  $MEASURERMS = "$CIRFILE.ma$maSuffix";
  #print "$fc $MEASURERMS $GOLDRMS $absTol $relTol $zeroTol\n";
  `$fc $MEASURERMS $GOLDRMS $absTol $relTol $zeroTol > $CIRFILE.errmsg.out 2> $CIRFILE.errmsg.err`;
  my $retval=$? >> 8;
