...
# to stdout to the "gold" stdout in the $GSFILE (DerivTestGSfile).  This
...
foreach $stepNum (1 .. $numSteps)
{
  ...
  # with the "Gold" .maX file, which is in OutputData/MEASURE_AC/STEP/DerivTest.cir.maX
  # Check that the Gold .maX file exists
  $maSuffix=$stepNum-1;
  $derivString= "ma$maSuffix";
  $GOLDDERIV = $GOLDPRN;
  $GOLDDERIV =~ s/prn$/$derivString/;
  #print "GOLDDERIV file = $GOLDDERIV\n";
  $maSuffix = $stepNum-1;
  if (not -s "$GOLDDERIV" )
  {
    print "GOLD $derivString file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  # compare gold and measured .ma files, for this step.
  $MEASUREDERIV = "$CIRFILE.ma$maSuffix";
  #print "$fc $MEASUREDERIV $GOLDDERIV $absTol $relTol $zeroTol\n";
  `$fc $MEASUREDERIV $GOLDDERIV $absTol $relTol $zeroTol > $CIRFILE.errmsg.out 2> $CIRFILE.errmsg.err`;
  my $retval=$? >> 8;
