...
# to stdout to the "gold" stdout in the $GSFILE (IntegTestGSfile).  This
...
$GSFILE="IntegTestACGSfile";
$absTol=1e-2; # large absTol because some measure values are around 1e3
$relTol=1e-3;
$zeroTol=1e-10;
...
foreach $stepNum (1 .. $numSteps)
{
  ...
  # with the "Gold" .maX file, which is in OutputData/MEASURE_AC/STEP/IntegTest.cir.maX
  # Check that the Gold .maX file exists
  $maSuffix=$stepNum-1;
  $integString= "ma$maSuffix";
  $GOLDINTEG = $GOLDPRN;
  $GOLDINTEG =~ s/prn$/$integString/;
  #print "GOLDINTEG file = $GOLDINTEG\n";
  $maSuffix = $stepNum-1;
  if (not -s "$GOLDINTEG" )
  {
    print "GOLD $integString file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  # compare gold and measured .ma files, for this step.
  $MEASUREINTEG = "$CIRFILE.ma$maSuffix";
  #print "$fc $MEASUREINTEG $GOLDINTEG $absTol $relTol $zeroTol\n";
  `$fc $MEASUREINTEG $GOLDINTEG $absTol $relTol $zeroTol > $CIRFILE.errmsg.out 2> $CIRFILE.errmsg.err`;
  my $retval=$? >> 8;
