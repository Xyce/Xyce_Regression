...
# to stdout to the "gold" stdout in the $GSFILE (ErrorFuncTestGSfile).  This
...
foreach $stepNum (1 .. $numSteps)
{
  ...
  # with the "Gold" .maX file, which is in OutputData/MEASURE_AC/ErrorFuncTest.cir.maX
  # Check that the Gold .maX file exists
  $maSuffix=$stepNum-1;
  $maxString= "ma$maSuffix";
  $GOLDMAX = $GOLDPRN;
  $GOLDMAX =~ s/prn$/$maxString/;
  #print "GOLDMAX file = $GOLDMAX\n";
  $maSuffix = $stepNum-1;
  if (not -s "$GOLDMAX" )
  {
    print "GOLD $maxString file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }
  ...
  #print "$fc $MEASUREMAX $GOLDMAX $absTol $relTol $zeroTol\n";
  `$fc $MEASUREMAX $GOLDMAX $absTol $relTol $zeroTol > $CIRFILE.errmsg.out 2> $CIRFILE.errmsg.err`;
  my $retval=$? >> 8;
