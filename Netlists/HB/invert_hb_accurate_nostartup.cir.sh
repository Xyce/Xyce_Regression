...
system("rm -f $CIRFILE.out $CIRFILE.err $CIRFILE.HB.* $CIRFILE.hb_ic.*");

# this file should not have been made, but remove it if it was
system ("rm -f $CIRFILE.startup.prn");
...
# this file should not be made
if ( -f "$CIRFILE.startup.prn" ) {
    print STDERR "Output file $CIRFILE.startup.prn made, when it shouldn't\n";
    $xyceexit=2;
}
