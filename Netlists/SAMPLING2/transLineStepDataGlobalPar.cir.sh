...
$XYCE=$ARGV[0];
#$XYCE_VERIFY=$ARGV[1];
$CIRFILE="transLineStepDataGlobalPar.cir";
...
$CIRSENSPRN=$CIRFILE;
$CIRSENSPRN =~ s/\.cir/\.cir\.SENS\.prn/g;
...
$CIRSENSPRN_save=$CIRSENSPRN."_save";

`mv $CIRPRN $CIRPRN_save`;
`mv $CIRSENSPRN $CIRSENSPRN_save`;
...
{
    $passed=0;
    print "diff failed for the $CIRPRN files\n";
}
$CMD="diff $CIRSENSPRN_save $CIRSENSPRN > $CIRSENSPRN.out 2> $CIRSENSPRN.err";
if (system("$CMD") != 0)
{
    $passed=0;
    print "diff failed for the $CIRSENSPRN files\n";
}
