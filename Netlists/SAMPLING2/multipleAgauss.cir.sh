...
$REFCIRPRN=$REFCIRFILE;
$REFCIRPRN =~ s/\.cir/\.cir\.prn/g;
...
{
    if (-z "$REFCIRFILE.err" ) {`rm -f $REFCIRFILE.err`;}
}

if (defined ($xyceexit)) {print "Exit code = 10\n"; exit 10;}

# Now diff the new output with the old --- match should be exact
$CMD="diff $CIRPRN $REFCIRPRN > $CIRPRN.out 2> $CIRPRN.err";
$passed=1;
if (system("$CMD") != 0)
{
    $passed=0;
