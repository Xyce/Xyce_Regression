...
$VERILOG_SOURCES="toys/testBacklog527.va";

print "Building the plugin...\n";

$result=system("$BUILDXYCEPLUGIN -o testBacklog527 $VERILOG_SOURCES .");
if ($result != 0)
{
...
$PLUGINPATH="$TESTROOT/testBacklog527.so";
...
#Generate a fake gold standard with the final column all zeros
open(INPUT,"$CIRFILE.prn");
open(OUTPUT,">$GOLDPRN");
while ($line = <INPUT>)
{
    # Handle first and last lines: The first line begins with "INDEX", the
    # last with "End"
    if ($line =~ m/^[IE]/)
    {
        print OUTPUT "$line";
    ...
    @s = split(" ",$line);
    # Column 0 is the Index column, column 1 is time
    $a = 0.0;
    printf OUTPUT "%3d %14.8e %14.8e %14.8e %14.8e %14.8e\n",$s[0],$s[1],$s[2],$s[3],$s[4],$a;
}
close(INPUT);
close(OUTPUT);
