...
$toynum=0;
for my $toy (@VERILOG_SOURCES)
{

    $result=system("$BUILDXYCEPLUGIN -o toy$toynum $toy .");
    if ($result != 0)
    {
...
$PLUGINPATH="$TESTROOT/toy0.so,$TESTROOT/toy1.so,$TESTROOT/toy2.so,$TESTROOT/toy3.so";
