...
# to stdout to the "gold" stdout in the $GSFILE (EquationEvalTestNoiseGSfile).  This
...
$GSFILE="EquationEvalTestNoiseGSfile";
$absTol=1e-5;
$relTol=1e-3;
$zeroTol=1e-10;
...
foreach $stepNum (1 .. $numSteps)
{
  ...
  # with the "Gold" .maX file, which is in OutputData/MEASURE_NOISE/STEP
  # Check that the Gold .maX file exists
  $maSuffix=$stepNum-1;
  $avgString= "ma$maSuffix";
  $GOLDAVG = $GOLDPRN;
  $GOLDAVG =~ s/prn$/$avgString/;
  #print "GOLDAVG file = $GOLDAVG\n";
  $maSuffix = $stepNum-1;
  if (not -s "$GOLDAVG" )
  {
    print "GOLD $avgString file does not exist\n";
    print "Exit code = 17\n";
    exit 17;
  }

  # compare gold and measured .ma files, for this step.
  $MEASUREAVG = "$CIRFILE.ma$maSuffix";
  #print "$fc $MEASUREAVG $GOLDAVG $absTol $relTol $zeroTol\n";
  `$fc $MEASUREAVG $GOLDAVG $absTol $relTol $zeroTol > $CIRFILE.errmsg.out 2> $CIRFILE.errmsg.err`;
  my $retval=$? >> 8;
