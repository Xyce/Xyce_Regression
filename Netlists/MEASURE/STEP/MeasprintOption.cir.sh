...
# gold mt1 file will be OutputData/MEASURE/MeasprintOption.cir.mt1
$GOLDMT0 = $GOLDPRN;
$GOLDMT1 = $GOLDPRN;
$GOLDMT0 =~ s/prn$/mt0/;
$GOLDMT1 =~ s/prn$/mt1/;
if ( (not -s "$GOLDMT0" ) || (not -s "$GOLDMT1" ) )
{ 
  print "GOLD .mt0 or .mt1 file does not exist\n";
  print "Exit code = 17\n"; 
  exit 17; 
}
...
foreach $idx (0 .. 3)
{  
  system("rm -f $CIR[$idx].mt* $CIR[$idx].out $CIR[$idx].err* $CIR[$idx].remeasure*");
}
...
foreach $idx (0 .. 3)
{
  ...
  # Did we make the measures file for the ALL and BOGO netlists?
  # Did we not make a .mt0 and .mt1 file for the STDOUT and NONE netlists?
  if ($idx < 2)
  {
    if ( (not -s "$CIR[$idx].mt0") || (not -s "$CIR[$idx].mt1") )
    {
      print "Failed to make .mt0 or .mt1 file for netlist $CIR[$idx]\n"; 
      print "Exit code = 17\n"; exit 17; 
    }
  }
  elsif ( (-s "$CIR[$idx].mt0") || (-s "$CIR[$idx].mt1") ) 
  {
  ...
  # mt0 and mt1 files should only have been made for ALL and BOGO netlists.
  # This was checked earlier.
  if ($idx < 2)
  {
    # compare gold and measured .mt0 and .mt1 files
    $MEASUREMT0 = "$CIR[$idx].mt0";
    $MEASUREMT1 = "$CIR[$idx].mt1";
    $CMD0="$fc $MEASUREMT0 $GOLDMT0 $absTol $relTol $zeroTol > $CIR[$idx].errmsg.out 2> $CIR[$idx].errmsg.err";
    $CMD1="$fc $MEASUREMT1 $GOLDMT1 $absTol $relTol $zeroTol > $CIR[$idx].errmsg.out 2> $CIR[$idx].errmsg.err";
    if ( (system($CMD0) != 0 ) || (system($CMD1) != 0 ))
    {
      print STDERR "test failed comparison of Gold and measured .mt files for netlist $CIR[$idx]\n";
      print "Exit code = 2\n";
      exit 2;
    }
    else
    {
      print "Passed comparison of .mt$stepNum files for netlist $CIR[$idx]\n";
    }
  }
  ...
  if ($idx < 2) 
  { 
    move("$CIR[$idx].mt0","$CIR[$idx].temp.mt0"); 
    move("$CIR[$idx].mt1","$CIR[$idx].temp.mt1");
  }
  ...
  move("$CIR[$idx].mt1","$CIR[$idx].remeasure.mt1");
  if ($idx < 2) 
  { 
    move("$CIR[$idx].temp.mt0","$CIR[$idx].mt0");
    move("$CIR[$idx].temp.mt1","$CIR[$idx].mt1");  
  }

  $CMD0 = "$fc $CIR[$idx].remeasure.mt0 $GOLDMT0 $absTol $relTol $zeroTol > $CIR[$idx].remeasure.mt0.out 2> $CIR[$idx].remeasure.mt0.err";  
  $CMD1 = "$fc $CIR[$idx].remeasure.mt1 $GOLDMT1 $absTol $relTol $zeroTol > $CIR[$idx].remeasure.mt1.out 2> $CIR[$idx].remeasure.mt1.err"; 
  if ( (system($CMD0) != 0) ||  (system($CMD1) != 0) )
  {
    print "Re-measure failed for netlist $CIR[$idx]\n"; 
    print "Exit code = 2\n"; exit 2;  
