...
@CIR;
$CIR[0]="MeasoutOptionZero.cir";
$CIR[1]="MeasoutOptionOne.cir";
$CIR[2]="MeasoutOptionBogo.cir";

# use one GSfile for all of the netlists
$GSFILE = "MeasoutOptionGSfile";
$REMGSFILE="MeasoutOptionRemeasureGSfile";

# default return code
$retval = 0;

# We only need .mt0 file OutputData/MEASURE/ for the One case.  The .mt0 
# file for the Bogo case should then be identical to the One case.
$dirname = dirname($GOLDPRN);
$GOLDMT0 = "$dirname/$CIR[1].mt0";
if (not -s "$GOLDMT0" ) 
{ 
  print "$GOLDMT0 file does not exist\n";
  print "Exit code = 17\n";
  exit 17; 
}
...
foreach $idx (0 .. 2)
{
  ...
  # Did we make a measure file for the One and Bogo cases.  And not make
  # a measure file for the Zero case.
  if ($idx == 0)
  {
    if (-s "$CIR[$idx].mt0") 
    {
  ...
  # compare gold and measured .mt0 files.  Note that no .mt0 was made for 
  # the Zero case, and that was checked above.
  $MEASUREMT0 = "$CIR[$idx].mt0";
  
  if ($idx == 1)
  {
    # Use file_compare since the .mt0 file for the One case has numbers in it
  ...
  {
    # .mt0 file for the One and Bogo cases should match exactly, since the Bogo case 
    # defaults to the One case.
    $CMD="$xpd $MEASUREMT0 $GOLDMT0";
    if ( system($CMD) != 0 )
    {
      print STDERR "test failed comparison of Gold and measured .mt0 files for netlist $CIR[$idx]\n";
      print "Exit code = 2\n";
  ...
  @searchstrings = ("Netlist warning: .OPTIONS MEASURE MEASOUT value must be 0 or 1. Setting value",
                    "to 1");
  if ($idx == 2) 
  { 
  ...
  # rename the .mt0 files that were made for the One and BOGO netlists
  use File::Copy;
  if ($idx > 0) { move("$CIR[$idx].mt0","$CIR[$idx].temp.mt0"); }


  # here is the command to run xyce with remeasure.  Use the .prn file
  # from the Zero netlist so the comparison of stdout is easier.
  ...
  # rename the mt0 files and compare them.  Use the Gold .mt0 file from the One 
  # netlist in this comparison as the "gold file"
  move("$CIR[$idx].mt0","$CIR[$idx].remeasure.mt0");
  if ($idx > 0) { move("$CIR[$idx].temp.mt0","$CIR[$idx].mt0"); }
