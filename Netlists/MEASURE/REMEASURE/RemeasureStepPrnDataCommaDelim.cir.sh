...
# run Xyce and check for the proper files.  This Xyce invocation produces
# a .PRN file with comma as the delimiter between columns.
$CMD="$XYCE -delim COMMA $CIRFILE > $CIRFILE.out";
$retval = system($CMD);
...
$retval = MeasureCommon::checkRemeasure($XYCE,$XYCE_VERIFY,$CIRFILE,$absTol,$relTol,$zeroTol,"prn",2);
