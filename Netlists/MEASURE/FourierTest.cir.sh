...
my @AtVals = (0.0, 0.0);
my @NumFreqVals = (0.0, 0.0);
my @GridSizeVals = (0.0, 0.0);
my @measureNames = ("", "");
while( $line=<NETLIST> )
{
  if( $line =~ /^\.measure/i)
  {  
    ...
    foreach $word (@words)
    {
      if( $word =~ "=" )
      {
        ...
        if( $keyword eq "AT" )
        {
          @AtVals[$numMeasures-1] = $value;
        }
        elsif( $keyword eq "NUMFREQ" )
        {
          @NumFreqVals[$numMeasures-1] = $value;
        }
        elsif( $keyword eq "GRIDSIZE" )
        {
          @GridSizeVals[$numMeasures-1] = $value;
        }
        elsif( $keyword eq "TO" )
        {
