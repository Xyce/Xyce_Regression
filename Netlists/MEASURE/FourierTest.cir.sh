...
# used for testing .OPTIONS MEASFAIL
...
foreach $i (0 .. 1)
{
  system("rm -f $CIR[$i].mt* $CIR[$i].out $CIR[$i].prn $CIR[$i].err");
}
...
# to stdout to the "gold" stdout in the $GSFILE (FourierTestGSfile).
...
$retval = MeasureCommon::checkNumberFormatinFourOutput("$CIRFILE.mt0",6);
if ( $retval != 0 )
{
...
$retval = MeasureCommon::checkNumberFormatinFourOutput("$CIRFILE.errmsg",6);
if ( $retval != 0 )
{
