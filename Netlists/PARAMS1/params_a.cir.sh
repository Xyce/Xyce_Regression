...
$LET=$CIRFILE;
$LET =~ s/params_([a-z])\.cir/$1/;

$cirfiles = `ls params_${LET}[0-9].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  $CMD = "diff params_${LET}0.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY params_${LET}0.cir params_${LET}0.cir.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
...
{ 
  `echo "Test Failed!" >> params_$LET.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "params_$LET.cir.prn.err" ) { `rm -f params_$LET.cir.prn.err`; }
`echo "Test Passed!" >> params_$LET.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
