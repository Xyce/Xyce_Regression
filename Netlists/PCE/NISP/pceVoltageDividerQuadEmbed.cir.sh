...
$XYCE=$ARGV[0];
#$XYCE_VERIFY=$ARGV[1];
$CIRFILE="pceVoltageDividerQuadEmbed.cir";
...
while ($num_tries < 2 && $finished == 0)
{
  ...
  $V1_analytical_mean = 3.99195;
  $V1_analytical_stddev = 0.254391;

  print STDERR  "\nTry # $num_tries :\n";

  $V1mean=`grep -m 1 'PCE mean of {v(1)}' $CIRFILE.out`;
  chomp($V1mean);
  $V1mean=~ s/\(embedded sampling\) projection PCE mean of \{v\(1\)\} = ([0-9]*)/\1/;
  print STDERR  "v(1) PCE mean is $V1mean, analytical mean is $V1_analytical_mean\n";

  $V1stddev=`grep -m 1 'PCE stddev of {v(1)}' $CIRFILE.out`;
  chomp($V1stddev);
  $V1stddev=~ s/\(embedded sampling\) projection PCE stddev of \{v\(1\)\} = ([0-9]*)/\1/;
