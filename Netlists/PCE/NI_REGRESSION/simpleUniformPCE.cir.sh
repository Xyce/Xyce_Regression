...
while ($num_tries < 2 && $finished == 0)
{
  ...
  chomp($V1mean);
  $V1mean=~ s/\(traditional samping\) regression PCE mean of \{v\(1\)\} = ([0-9]*)/\1/;
  ...
  chomp($V1stddev);
  $V1stddev=~ s/\(traditional samping\) regression PCE stddev of \{v\(1\)\} = ([0-9]*)/\1/;
