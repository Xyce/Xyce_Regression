...
if ($retval1 != 0)
{
  print "Failed power test for NPN BJT, based on calculation\n";
  $retval=$retval1;
}

$pCalcColIdx = 10; # P dissipated by sources
$pXyceColIdx = 8; # P(q1) reported by Xyce for PNP
$wXyceColIdx = 9; # W(q1) reported by Xyce for PNP
...
if ($retval2 != 0)
{
  print "Failed power test for NPN BJT, based on source dissipation\n";
  $retval=$retval2;
}
