...
$pCalcColIdx = 5; # P(M1) calculated, from combined source and resistor dissipation
$pXyceColIdx = 2; # P(M1) reported by Xyce 
$wXyceColIdx = 3; # W(M1) reported by Xyce

# Do the comparison based on power balance.  Pass in a reference to @prnData
# The power for the MOSFET should the negative of the power dissipated in the
# two sources and two resistors
$retval2 = PowerCommon::comparePowerColumns(\@prnData,$pCalcColIdx,$pXyceColIdx,$wXyceColIdx,$absTol,$relTol,$zeroTol);
if ($retval2 != 0)
{
  print "Failed power test for MOSFET, based on power balance\n";
  $retval=$retval2;
}
