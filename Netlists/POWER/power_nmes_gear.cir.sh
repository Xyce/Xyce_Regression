...
$pCalcColIdx = 4; # P(Z1) calculated, via formula
$pXyceColIdx = 2; # P(Z1) reported by Xyce 
$wXyceColIdx = 3; # W(Z1) reported by Xyce

# Do the comparison for NPN.  Pass in a reference to @prnData
$retval1 = PowerCommon::comparePowerColumns(\@prnData,$pCalcColIdx,$pXyceColIdx,$wXyceColIdx,$absTol,$relTolCalc,$zeroTol);
if ($retval1 != 0)
{
  print "Failed power test for MOSFET, based on expected calculation result\n";
}

$pCalcColIdx = 5; # P(Z1) calculated, from combined source dissipation
$pXyceColIdx = 2; # P(Z1) reported by Xyce 
$wXyceColIdx = 3; # W(Z1) reported by Xyce
# Do the comparison based on combined source dissipation.  Pass in a reference to @prnData
$retval2 = PowerCommon::comparePowerColumns(\@prnData,$pCalcColIdx,$pXyceColIdx,$wXyceColIdx,$absTol,$relTolPowerBal,$zeroTol);
if ($retval2 != 0)
{
