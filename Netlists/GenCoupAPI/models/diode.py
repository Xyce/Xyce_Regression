...
class Device(BaseDevice):
    ...
    def computeXyceVectors(self, solV, fSV, stoV, t, deviceOptions, solverState,
        ...
        Vn  = solV[1];
        Vp  = solV[0];
        DEBUG and print("Vpp: ", Vpp)
        DEBUG and print("Vn: ", Vn)
        DEBUG and print("Vp: ", Vp)
        Vd = Vpp - Vn
        ...
        Isat *= IRfactor;
        DEBUG and print("Isat: ", Isat)

        if (Vd >= -3.0 * Vte):
            arg1 = Vd / Vte;
            arg1 = min(CONSTMAX_EXP_ARG, arg1);
            evd = np.exp(arg1);
    
            Id = Isat * (evd - 1.0) + gmin * Vd;
            Gd = Isat * evd / Vte + gmin;
            print("case1: True", Gd, "case2: False",0)
            ...
            print("case1: False", 0, "case2: True", Gd)
        ...
        #li_Pos = i_params["li_Pos"]
        #li_Neg = i_params["li_Neg"]
        #li_Pri = i_params["li_Pri"]
        ...
        #Fcontribs[li_Pos] -= coef;
        Fcontribs[0] -= coef;
    
        # load in the KCL for the negative node:
        coef = Id;
        #Fcontribs[li_Neg] -= coef;
        Fcontribs[1] -= coef;
        ...
        DEBUG and print("Vte = ",Vte)
        DEBUG and print("Id  = ",Id)
        DEBUG and print("Gd  = ",Gd)
        DEBUG and print("Cd  = ",Cd)

        for i in range(numVars):
            F[i] = Fcontribs[i]
            Q[i] = Qcontribs[i]
            #print("Q[i]:"%i,Q[i])
        DEBUG and print("F[0]:",F[0])
        DEBUG and print("F[1]:",F[1])
        DEBUG and print("F[2]:",F[0])
        #DEBUG and print("Q[1]:",Q[1])
        #DEBUG and print("Q[2]:",Q[0])
        ...
        dQdXcontribs = np.zeros(shape=(numVars,numVars),dtype=np.float64)
