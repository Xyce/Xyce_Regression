...
system("rm -f $CIRFILE.NOISE.* $CIRFILE.out $CIRFILE.err");
...
# now compare the test and gold file .prn files $retcode=0;
...
$CMD="$XYCE_ACVERIFY $CIRFILE.NOISE.prn $GOLDPRN.NOISE.prn $absTol $relTol $zeroTol $freqreltol > $CIRFILE.NOISE.prn.out 2> $CIRFILE.NOISE.prn.err";
$retval = system("$CMD");
$retval = $retval >> 8;
if ($retval != 0)
{
