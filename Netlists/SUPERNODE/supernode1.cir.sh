...
$cirfiles = `ls supernode1.cir supernode1[a-z].cir 2> /dev/null`;
...
foreach $cirname (@cirlist)
{
  $CMD = "diff supernode1.cir.prn $cirname.prn > $cirname.prn.out 2> $cirname.prn.err";
  if ( system("$CMD") != 0 ) 
  { 
    $CMD = "$XYCE_VERIFY supernode1.cir supernode1.cir.prn $cirname.prn >> $cirname.prn.out 2>> $cirname.prn.err";
...
{ 
  `echo "Test Failed!" >> supernode1.cir.prn.out`;
  print "Exit code = 2\n"; exit 2; 
}

if ( -z "supernode1.cir.prn.err" ) { `rm -f supernode1.cir.prn.err`; }
`echo "Test Passed!" >> supernode1.cir.prn.out`;
print "Exit code = 0\n"; exit 0;
