...
# previous run failed.
system("rm -f $CIRFILE.mt0.* $DASHOFILE.prn* $DASHOFILE.mt0.*");
system("rm -f $DASHOFILE.prn*");

# run Xyce
$CMD="$XYCE -o $DASHOFILE $DASHOFILE.mt0 > $DASHOFILE.mt0.out 2>$DASHOFILE.mt0.err";
$retval=system($CMD);
...
if ( !(-f "$DASHOFILE.mt0.mt0") ){
  print STDERR "Missing -o output file for .MEASURE TRAN, $DASHOFILE.mt0.mt0\n";
  $xyceexit=2;
}
