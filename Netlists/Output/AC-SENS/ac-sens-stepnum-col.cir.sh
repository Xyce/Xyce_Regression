...
if ( not -s "$CIRFILE.FD.SENS.noindex.prn")
{
  print STDERR "Missing output file $CIRFILE.FD.SENS.noindex.prn\n";
  $xyce_exit = 14;
}

if ( not -s "$CIRFILE.FD.SENS.gnuplot.prn")
{
  print STDERR "Missing output file $CIRFILE.FD.SENS.gnuplot.prn\n";
  $xyce_exit = 14;
}
...
# now compare the test and gold .prn files for .PRINT SENS
$retcode=0;
...
$CMD="$XYCE_ACVERIFY $CIRFILE.FD.SENS.noindex.prn $GOLDPRN.FD.SENS.noindex.prn $absTol $relTol $zeroTol $freqreltol > $CIRFILE.FD.SENS.noindex.prn.out 2> $CIRFILE.FD.SENS.noindex.prn.err";
$retval = system("$CMD");
$retval = $retval >> 8;
if ($retval != 0)
{
  print STDERR "Comparator exited with exit code $retval on file $CIRFILE.FD.SENS.noindex.prn\n";
  $retcode = 2;
}

$CMD="$XYCE_ACVERIFY $CIRFILE.FD.SENS.gnuplot.prn $GOLDPRN.FD.SENS.gnuplot.prn $absTol $relTol $zeroTol $freqreltol > $CIRFILE.FD.SENS.gnuplot.prn.out 2> $CIRFILE.FD.SENS.gnuplot.prn.err";
$retval = system("$CMD");
$retval = $retval >> 8;
if ($retval != 0)
{
  print STDERR "Comparator exited with exit code $retval on file $CIRFILE.FD.SENS.gnuplot.prn\n";
  $retcode = 2;
}
