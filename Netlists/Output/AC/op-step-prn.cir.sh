...
system("rm -f $CIRFILE.out $CIRFILE.err $CIRFILE.TD.* $CIRFILE.FD.prn $CIRFILE.res");
...
$CMD="$XYCE_VERIFY $CIRFILE $GOLDPRN.prn $CIRFILE.TD.prn --goodres=$GOLDDIR/$CIRFILE.res > $CIRFILE.TD.prn.out 2> $CIRFILE.TD.prn.err";
if (system("$CMD") != 0) {
