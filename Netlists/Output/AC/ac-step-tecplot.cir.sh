...
system("rm -f $CIRFILE.FD.dat $CIRFILE.TD.dat $CIRFILE.out $CIRFILE.err");
system("rm -f $CIRFILE.FD.dat.out $CIRFILE.FD.dat.err $CIRFILE.prn.*");
...
$retcode = testFileBasic("$CIRFILE.FD.dat");
print "Done with call testFileBasic!  At this point retcode = $retcode\n";

# convert and compare the *FD.dat file.
$result = system( "$TRANSLATE $CIRFILE.FD.dat");
if ( $result != 0 )
{
...
{
  $CMD="$XYCE_ACVERIFY --skipfooter $GOLDDAT.FD.prn test_$CIRFILE.FD.dat $abstol $reltol $zerotol $freqreltol > $CIRFILE.FD.dat.out 2> $CIRFILE.FD.dat.err";
  if (system("$CMD") != 0) {
       print STDERR "Verification failed on file $CIRFILE.FD.dat vs $GOLDDAT.FD.prn, see $CIRFILE.FD.dat.err/out\n";
       $retcode = 2;
  }
}

# the test should not make a .TD.dat file
if ( -f "$CIRFILE.TD.dat") 
{
    print "Output file $CIRFILE.TD.dat produced, when it should not\n";
    $retcode = 2;
}
