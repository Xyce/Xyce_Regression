...
system("rm -f $CIRFILE.FD.csv $CIRFILE.TD.csv $CIRFILE.out $CIRFILE.err $CIRFILE.prn.*");
...
# the test should not make a .TD.csv file
if ( -f "$CIRFILE.TD.csv" ) 
{
    print STDERR "$CIRFILE.TD.csv produced, when it should not\n";
    $retcode = 2;
}
